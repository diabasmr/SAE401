...
#[AsCommand(
    name: 'app:import:region',
    description: 'Import des régions depuis un CSV'
)]
class ImportRegionCommand extends Command
{
    ...
    {
        $this
            ->setDescription('Import des régions depuis un CSV')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        while (($row = fgetcsv($handle, 0, ',')) !== false) { //tant qu'on a une ligne
            ...
            $codeRegion = $this->formatCodeRegion($data['code_region']); //formate le codeRegion
            ...
            // Création de la région en tant qu'objet symfony
            $reg = new Region();
            $reg->setCode($codeRegion); //pas de changement de type car donnees deja en string
            $reg->setNom($data['nom_region']);

            $this->em->persist($reg); //mise en attente memoire
...
// LANCER AVEC : php bin/console app:import:region src/Command/stats.csv